...
# The registry is one of: - a local/mounted directory containing plain,
# already-unzipped recipe folders at "<registry>/<letter>/<n>/<version>/". -
# "git+https://..." or "git+ssh://..." — a git repo containing plain,
# already-unzipped recipe folders in the same layout. Fetched via a partial,
# sparse clone (--filter=blob:none --sparse), so only the one
# "<letter>/<n>/<version>/" subtree is actually downloaded — not the whole repo
# or its history. - a plain "http://" or "https://" base URL serving one tarball
# per (name, version) at "<registry>/<letter>/<n>/<version>.tar.gz". Detected
# automatically from the string's shape; see _spm_resolve_recipe_dir.
set(SPM_REGISTRY
...
# semicolon-separated "Key: Value" strings. NOTE: this is a CACHE variable and
# will be written to CMakeCache.txt in plain text — for secrets, prefer passing
# -DSPM_REGISTRY_HEADERS=... on the command line each run (not committed), or
# override per-package via spm_require_package(HEADERS ...).
...
function(
  ...
  if(IS_DIRECTORY "${registry}")
    # Local / mounted directory: use in place
  ...
  elseif(registry MATCHES "^https?://")
    # Remote registry: one tarball per (name, version)
...
function(_spm_list_local_versions name out_var)
  ...
  if(EXISTS "${_pkg_dir}")
    ...
    foreach(_entry ${_entries})
      if(IS_DIRECTORY "${_pkg_dir}/${_entry}"
         AND EXISTS "${_pkg_dir}/${_entry}/CMakeLists.txt")
...
macro(_spm_run_tests)
  ...
  if(SPM_SKIP_TESTS)
    _spm_log(
      "RUN_TESTS requested for '${name}@${version}' but SPM_SKIP_TESTS is ON, skipping"
    )
    return()
  endif()

  set(_test_script "${recipe_dir}/Test.cmake")
  if(EXISTS "${_test_script}")
    ...
    _spm_log("Running Test.cmake for '${B_NAME}@${B_VERSION}'")
    include("${_test_script}")
    return()
  endif()

  if(NOT CTEST_EXECUTABLE)
    _spm_log_fatal(
      "RUN_TESTS requested for '${B_NAME}@${B_VERSION}' but no ctest executable was found"
    )
  else()
    ...
    if(NOT _test_result EQUAL 0)
      _spm_log_fatal(
        "Test phase failed for '${B_NAME}@${B_VERSION}' — refusing to cache a "
...
macro(_spm_run_install)
  ...
  _spm_log("Installing '${B_NAME}@${B_VERSION}' - ${B_CACHE_DIR}")

  set(_install_script "${recipe_dir}/Install.cmake")
  if(EXISTS "${_install_script}")
    set(SPM_IMPORT_NAME "${B_IMPORT_NAME}")
    set(SPM_IMPORT_VERSION "${B_VERSION}")
    set(SPM_IMPORT_CACHE_DIR "${B_CACHE_DIR}")
    set(SPM_IMPORT_SHARED "${B_SHARED}")
    _spm_log("Running Install.cmake for '${B_NAME}@${B_VERSION}'")
    include("${_install_script}")
    return()
  endif()
  ...
  if(NOT _install_result EQUAL 0)
    _spm_log_fatal(
      "Install failed for '${B_NAME}@${B_VERSION}':\n${_install_output}")
  endif()

  file(GLOB_RECURSE _installed_files "${B_CACHE_DIR}/*")
  if(NOT _installed_files)
    _spm_log_fatal(
      "Recipe for '${B_NAME}@${B_VERSION}' configured and built successfully but "
      "installed nothing into ${B_CACHE_DIR}. Check that the recipe enables the "
...
macro(_spm_load_default_patches out_var)
  ...
  set(_recipe_patches_dir "${B_RECIPE_DIR}/patches")
  if(IS_DIRECTORY "${_recipe_patches_dir}")
    file(
      GLOB _discovered_patches
...
# packages/cache/<l>/<n>/<hash>/, and expose it as an imported target.
function(_spm_build_and_import name version recipe_dir)
  ...
  if(EXISTS "${_stamp_file}"
    ...
    _spm_log(
      "'${name}@${version}' already built (${_hash}), reusing ${_cache_dir}")
  else()
    if(EXISTS "${_cache_dir}")
      _spm_log(
        "Forcing rebuild of '${name}@${version}' (${_hash}), clearing stale cache"
      )
...
")
    ...
    if(CMAKE_TOOLCHAIN_FILE)
      ...
      list(APPEND _toolchain_args -DCMAKE_C_COMPILER=${CMAKE_C_COMPILER}
    ...
    execute_process(
      COMMAND
        ...
        -DCMAKE_BUILD_TYPE=${_pkg_build_type} ${_toolchain_args}
    ...
    if(NOT _cfg_result EQUAL 0)
      _spm_log_fatal(
        "Configure failed for '${name}@${version}':\n${_cfg_output}")
    endif()
...
#
# spm_require_package( NAME       spdlog VERSION    1.14.1
#
# GIT_URL    <override recipe's default> GIT_TAG    <override recipe's default>
# URL        <override recipe's default> HASH       <override recipe's default>
#
# DIRECTORY  <use an explicit local recipe dir instead of resolving via
# SPM_PACKAGES_DIR/SPM_REGISTRY>
#
# REGISTRY   <override SPM_REGISTRY for this package only — a local/mounted
# directory, a "git+https(s)/ssh" repo (sparse partial clone), or an http(s)
# tarball server> REGISTRY_REF <branch/tag for a git+ registry override, if not
# the repo's default branch> HEADERS    "Authorization: Bearer xyz" "X-Custom:
# value" (override SPM_REGISTRY_HEADERS for this package only; used for http(s)
# tarball downloads and git+ clones, ignored in local-directory registry mode)
#
# PATCHES    a.patch b.patch CONFIGS    "SOME_OPTION=ON" "OTHER_OPTION=OFF"
#
# RUN_TESTS  # run the recipe's own ctest suite before caching FORCE      #
# ignore any existing cache entry, rebuild SHARED     # build a shared library
# instead of static (default) IMPORT_NAME <real CMake package/target name> # use
# when a package's actual CMake package name (what # find_package() looks for)
# differs from the recipe's # own identity/folder name — e.g. NAME abseil
# installs # as CMake package "absl", so pass IMPORT_NAME absl. # Defaults to
# NAME if not given. )
#
function(spm_require_package)
  ...
  if(NOT ARG_VERSION AND NOT ARG_DIRECTORY)
    _spm_log_fatal(
      "VERSION argument is required (or pass DIRECTORY explicitly)")
  endif()
  ...
  _spm_build_and_import(
    ...
    ${_force_flag}
    ${_shared_flag})
  if(NOT TARGET ${_effective_import_name}
     AND NOT TARGET ${_effective_import_name}::${_effective_import_name})
    _spm_log(
...
# Called FROM A RECIPE (running as its own isolated project) to fetch upstream
# source: either GIT_URL/GIT_TAG (git clone), or URL/HASH (download + hash
# validated). Caller overrides passed down from spm_require_package()
# (SPM_PKG_*) win over the recipe's own defaults given here.
function(spm_fetch_source)
  ...
  elseif(_url)
    if(NOT _hash)
      _spm_log_fatal(
        "URL source for '${SPM_PKG_NAME}' requires a HASH for validation")
    endif()
    FetchContent_Declare(
      ${SPM_PKG_NAME}
      URL ${_url}
    ...
    _spm_log_fatal(
      "spm_fetch_source() needs GIT_URL or URL for '${SPM_PKG_NAME}'")
  endif()
  ...
  foreach(_patch ${SPM_PKG_PATCHES})
    if(NOT GIT_EXECUTABLE)
      _spm_log_fatal(
        "Recipe '${SPM_PKG_NAME}' needs a patch applied, but no git executable was found"
      )
    endif()
    ...
    if(NOT _patch_result EQUAL 0)
      _spm_log_fatal(
        "Failed to apply patch '${_patch}' to '${SPM_PKG_NAME}':\n${_patch_output}"
      )
    endif()
  endforeach()

  set(SPM_PKG_SOURCE_DIR
      "${${SPM_PKG_NAME}_SOURCE_DIR}"
      PARENT_SCOPE)
endfunction()

#
# spm_run_external_build( [SOURCE_DIR <dir>]              # default:
# SPM_PKG_SOURCE_DIR [CONFIGURE_COMMAND <cmd...>] BUILD_COMMAND <cmd...>
# [INSTALL_COMMAND <cmd...>] [BUILD_IN_SOURCE] )
function(spm_run_external_build)
  ...
  cmake_parse_arguments(X "${options}" "${oneValArgs}" "${multiValArgs}"
                        ${ARGN})

  if(NOT X_SOURCE_DIR)
    if(NOT SPM_PKG_SOURCE_DIR)
      _spm_log_fatal(
        "spm_run_external_build() needs SOURCE_DIR (call spm_fetch_source() first, or pass it explicitly)"
      )
    endif()
