...
function(
    ...
    if(EXISTS "${_local_repo_dir}/CMakeLists.txt")
        spm_log("Using recipe for '${name}@${version}' at ${_local_repo_dir}")
        set(${out_dir}
            "${_local_repo_dir}"
            PARENT_SCOPE)
        return()
    ...
    if(IS_DIRECTORY "${registry}")
        ...
        spm_log("Using recipe for '${name}@${version}' directly from local registry ${_src}")
        set(${out_dir}
            "${_src}"
            PARENT_SCOPE)
        return()
    ...
    elseif(registry MATCHES "^git\\+(.+)$")
        ...
        spm_log("Sparse-cloning '${_path_in_repo}' from ${_git_url}")
        spm_execute_process(
            COMMAND
            ${GIT_EXECUTABLE}
            ${_git_config_args}
    ...
    elseif(registry MATCHES "^https?://")
        ...
        spm_log("Fetching '${name}@${version}' from ${_tarball_url}")
        file(
...
function(spm_require_package)
    ...
    if(EXISTS "${_unsupported_script}")
        ...
        if(SPM_UNSUPPORTED_REASON)
            spm_log("Recipe for '${ARG_NAME}@${ARG_VERSION}' is unsupported here: ${SPM_UNSUPPORTED_REASON}")
            return()
        endif()
    endif()
